...
exportAudioInfo: function() {
	try {
		...
		var audioInfo = [];
		var audioTracks = sequence.audioTracks; // Get the audio tracks from the sequence
				...
				for (var j = 0; j < currentTrack.clips.numItems; j++) {
					...
					if (currentClip) {
						...
						if (currentClip.start) {
							var seconds = currentClip.start.seconds; // Access the seconds property of the start time object
							startTime = this.secondsToHms(seconds); // Convert seconds to hh:mm:ss format
						}
		...
		if (audioInfo.length > 0) {
			// Convert the audio info to a string
			var audioInfoString = "";
			for (var i = 0; i < audioInfo.length; i++) {
				var info = audioInfo[i];
				audioInfoString += info.startTime + " " + info.name + "\n";
			}

			var filelocation = Folder.userData.fsName + "/audio_info.txt";
			// Save the audio info to a text file
			var file = new File(filelocation);
			file.open("w");
			file.write(audioInfoString);
			file.close();

			$._PPP_.updateEventPanel("Audio information saved to " + filelocation);
		} else {
			$._PPP_.updateEventPanel("No audio information found");
		}

	} catch (error) {
		$._PPP_.updateEventPanel("An error occurred: " + error.message);
	}
}
}
